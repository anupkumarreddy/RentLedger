...
def dashboard_snapshot(landlord):
    ...
    # Overdue statuses were just refreshed, so the status filter is accurate.
    overdue_qs = overdue_installments_for_landlord(landlord)
    overdue_amount = overdue_qs.aggregate(total=Sum("outstanding_amount"))["total"] or Decimal("0")
